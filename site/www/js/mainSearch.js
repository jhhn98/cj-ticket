...
function resveItems(dataList) {
    ...
    dataList.forEach(function(item) {
		...
		const sttusMap = {
            'ING': { text: '접수중', classNm: 'apcType1' },
            'WAIT':  { text: '접수대기', classNm: 'apcType2' },
            'CLOSE':  { text: '접수마감', classNm: 'apcType3' }
        };
        ...
        const html = `
            <div class="slide-item" data-filter-keyword="${item.sttusText}">
                <a href="${item.linkUrlText}${item.resveNo}">
                    <i class="state ${item.sttusClass}">
                        <svg><use href="/site/www/images/main/sprite-main.svg#icon-chip"></use></svg>
