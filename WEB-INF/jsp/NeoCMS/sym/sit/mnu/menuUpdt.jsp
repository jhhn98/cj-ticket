...
<form:form modelAttribute="menu" name="menuForm" action="updateMenu.do" onsubmit="fn_ajax_submit(this.id);return false;">

    <form:hidden path="menuNo"/>
    <form:hidden path="siteId"/>
    <form:hidden path="upperMenuNo" id="select_upperMenuNo"/>
    ...
    <tbody>
        <tr>
            ...
            <td>
                <span id="upperMenuNm"><c:out value="${upperMenu.menuNm}"/></span>
                <a href="#n" class="iconTextButton search small marginLeft_10 upperMenuBtn"><span>상위 메뉴 변경</span></a>
                <p class="info_text" id="info_menu_text" style="display: none;">이동할 상위메뉴를 좌측 메뉴목록에서 선택하여주세요.</p>
            </td>
        </tr>
        <tr>
            <th scope="row" class="essential_input"><form:label path="menuNm">메뉴명</form:label></th>
            <td>
                <div class="form_element two_button">
                    <span class="element_wrap"><form:input path="menuNm" size="60"/><form:errors path="menuNm"/></span>
                    <span class="element_wrap button_area">
                        <a onclick="fn_deleteMenu(this.href); return false;" href="./deleteMenu.do?siteId=<c:out value="${menu.siteId}"/>&amp;menuNo=<c:out value="${menu.menuNo}"/>" class="iconTextButton delete small">메뉴 삭제</a>
                        <a onclick="fn_popWindow(this.href,'menuSearchPop','toolbors=no,scrollbars=no,width=300px,height=300px');  return false;" href="/common/getQRcode.do?key=<c:out value="${menu.menuNo}"/>" class="iconTextButton qrcode small"> QRCode</a>
                    </span>
                </div>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <div class="form_element full"><span class="element_wrap"><form:input path="cntntsNm" size="60"/><form:errors path="cntntsNm"/></span></div>
        ...
        <tr>
            ...
            <td>
                <div class="form_element full"><span class="element_wrap"><form:input path="subTitle" size="60"/><form:errors path="subTitle"/></span></div>
        ...
        <tr>
            ...
            <td>
                <div class="form_element">
                    <span class="element_wrap"><form:input path="paramtr" size="60"/> <form:errors path="paramtr"/></span>
                </div>
            </td>
        </tr>
        <tr>
            <th scope="row">실제 메뉴 URL</th>
            <td><span class="eng_text"><c:out value="${menu.menuUrl}"/></span></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <p class="info_text">담당자명을 임의로 입력하시려면 반드시 현재 등록된 담당자를 해제하셔야합니다.</p>
                <div class="manager_assign">
                    <div class="manager_input">
                        <div><input id="deptNm" name="deptNm" value="<c:out value="${menu.deptNm}"/>" <c:if test="${not empty menu.deptNm}">readonly="readonly"</c:if> size="20" placeholder="담당부서" title="담당부서"/></div>
                        <div><input id="emplNm" name="emplNm" value="<c:out value="${menu.emplNm}"/>" <c:if test="${not empty menu.emplNm}">readonly="readonly"</c:if> size="12" placeholder="담당자" title="담당자"/></div>
                        <div><input id="emplTelno" name="emplTelno" value="<c:out value="${menu.emplTelno}"/>" <c:if test="${not empty menu.emplTelno}">readonly="readonly"</c:if> size="12" placeholder="문의처" title="문의처"/></div>
                    </div>
...
<!-- 
        <tr>
            <th>콘텐츠작성자</th>
            <td colspan="2">
                <c:out value="${menu.cntntsWrterNm}"/>(<span class="em_red"><c:out value="${menu.cntntsWrterId}"/></span>)
                <span class="info">실제 작업자 정보입니다.</span>
            </td>
        </tr>
 -->
        <tr>
        ...
        <tr>
            ...
            <td>
                <form:hidden path="cntntsLastUpdusrPnttm" size="12"/>
                <span class="eng_text icon_text icon_date_edit"><c:out value="${menu.cntntsLastUpdusrPnttmYMD}"/></span>
                <p class="info_text">콘텐츠를 수정할 경우 자동으로 적용됩니다.</p>
            </td>
        </tr>
    </tbody>
    </table>
...
<script nonce="NEOCMSSCRIPT">
    function fn_initCharger() {
        ...
        $("#deptNm").attr("readonly",false);
        $("#emplNm").attr("readonly",false);
        $("#emplTelno").attr("readonly",false);
    }
...
<script nonce="NEOCMSSCRIPT">
    function selectEmployee(employee){
        ...
        $("#emplNm").attr("readonly",true);
        $("#emplTelno").attr("readonly",true);
    ...
    $(document).ready(function(){
        ...
        /** 직원 검색 버튼에 대한 액션 */
        $(".upperMenuBtn").click(function (e) {
            if(isMenuSelectMode == true){
                $(".menu_selector").removeClass("flag_on");
                $("#info_menu_text").hide();
                isMenuSelectMode = false;
                $("#upperMenuBtn").html("상위 메뉴 변경");
            }else{
                $(".menu_selector").addClass("flag_on");
                $("#info_menu_text").show();
                $("#upperMenuBtn").html("변경취소");
                isMenuSelectMode = true;
            }
        });
    });
</script>
