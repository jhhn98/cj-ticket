...
<script nonce="NEOCMSSCRIPT">
 ...
 <script nonce="NEOCMSSCRIPT">
	function index_up() {
		var form = document.getElementById('employeeSortForm');
...
<div class="marginLeft_30 marginRight_30">
    <h2 class="marginTop_0">직원 순서 변경</h2>
    <form name="employeeSortForm" id="employeeSortForm" method="post" action="./sortEmployee.do" onsubmit="return IdxSubmit(this)">

        <input type="hidden" name="deptCode" id="deptCode" value="<c:out value="${department.deptCode}"/>"/>
        <input type="hidden" name="emplCode" id="emplCode" value="<c:out value="${employee.emplCode}"/>"/>
    ...
    <c:forEach var="result" items="${employeeList}" varStatus="status">
        <option value="<c:out value="${result.emplCode}"/>"><c:out value="${result.emplNm}"/></option>
    </c:forEach>
        </select>
    </div>
