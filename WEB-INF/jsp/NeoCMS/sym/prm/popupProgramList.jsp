...
<c:set var="active_menu" value="PRG_1" scope="request"/>
    <script nonce="NEOCMSSCRIPT">
        function fn_prgSelect( prgNo, prgNm ) {
            $("#progrmNo").val(prgNo);
            $("#progrmNm").val(prgNm);
            closeSlideView();
        }
...
<div class="textARight marginBottom_5">
    <div class="table_search">
        <form:form modelAttribute="programVO" name="searchForm" method="get" action="/neo/popupProgramList.do" onsubmit="return fn_ajax_search_submit(this.id);return false;">
            ...
            <table>
                ...
                <c:forEach var="result" items="${resultList}" varStatus="status">
                    <tr>
                        ...
                        <td><a href="./selectProgramView.do?progrmNo=<c:out value="${result.progrmNo}"/>&amp;pageUnit=<c:out value="${programVO.pageUnit}"/>&amp;searchCnd=<c:out value="${programVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${programVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${programVO.pageIndex}"/>"><c:out value="${result.progrmNm}"/></a></td>
                        <td class="textACenter"><c:out value="${manageFnctEnncMap[result.manageFnctEnnc]}"/></td>
                        <td><c:out value="${result.progrmDc}"/></td>
                        <td class="textACenter"><a href="#fn_prgSelect" onclick="fn_prgSelect('<c:out value="${result.progrmNo}"/>', '<c:out value="${result.progrmNm}"/>')" class="iconTextButton small edit"><span>선택</span></a></td>
                    </tr>
...
<div class="textACenter marginTop_20">
    <ui:pagination paginationInfo = "${tsu:xssPagingFilter(paginationInfo)}" type="neoTreePop" jsFunction="./popupProgramList.do?pageUnit=${tsu:xssLinkFilter(programVO.pageUnit)}&amp;searchCnd=${tsu:xssLinkFilter(programVO.searchCnd)}&amp;searchKrwd=${tsu:xssLinkFilter(programVO.searchKrwd)}&amp;pageIndex=" />
</div>
