...
<div class="page_location">
	...
	<i class="sign_gt">메뉴 위치 구분 기호 ></i>
	<span class="depth4_title">월별접속로그</span>
</div>
<div class="tab_navigation">
	<a href="/neo/selectConectLogDalyList.do?siteId=<c:out value="${siteInfo.siteId}"/>" >일별 접속로그</a></li>
	<a href="/neo/selectConectLogMnbyList.do?siteId=<c:out value="${siteInfo.siteId}"/>" class="current">월별 접속로그</a></li>
	<a href="/neo/selectConectLogYabyList.do?siteId=<c:out value="${siteInfo.siteId}"/>" >년별 접속로그</a></li>
</div>
	...
	<div class="arrangementWrap data_info">
		...
		<div class="arrRight">
			<div class="table_search">
				<form name="conectLog" id="conectLog" method="post" action="./selectConectLogMnbyList.do">
					<input type="hidden" name="siteId" id="siteId" value="${conectLogVO.siteId}"/>
					<span class="srch_element text">
						<select name="sy" id="sy" class="select">
							<jsp:useBean id="now" class="java.util.Date"/>
							<fmt:formatDate value="${now}" pattern="yyyy" var="yearMax"/>
							<c:forEach begin="2019" end="${yearMax}" varStatus="idx">
					...
					<span class="srch_element select">
						<select name="sm" id="sm" class="select">
					...
					<span class="srch_element submit">
						<button type="submit" class="search_button">보기</button>
					</span>
				</form>
			</div>
		</div>
			...
			<div class="scroll_view">
				...
				<table>
					...
					<c:forEach var="conectLog" items="${conectLogList}" varStatus="status">
						<tr>
							<td class="textACenter"><c:out value="${conectLog.dateYM}"/></td>
							<td>
