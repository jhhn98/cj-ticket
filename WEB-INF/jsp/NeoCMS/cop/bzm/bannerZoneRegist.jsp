...
<div class="page_location">
	...
	<i class="sign_gt">메뉴 위치 구분 기호 ></i>
	<span class="depth4_title">배너존등록</span>
</div>
<div class="dataWrap">
    <form:form modelAttribute="bannerZone" name="bannerZoneForm" action="addBannerZone.do" onsubmit="return validateBannerZone(this)">
        <table>
            <colgroup>
                                        ...
                                        <optgroup label="<c:out value="${siteSeList.codeNm}"/>">
                                            <c:forEach var="siteInfoList" items="${siteInfoList}" varStatus="status">
                                                <c:if test="${siteSeList.code eq siteInfoList.siteSe}">
                                                    <option value="<c:out value="${siteInfoList.siteId}"/>" <c:if test="${bannerZone.siteId eq siteInfoList.siteId}">selected="selected"</c:if>><c:out value="${siteInfoList.siteNm}"/></option>
                                                </c:if>
                                            </c:forEach>
                                        </optgroup>
                                    </c:forEach>
            ...
            <tr>
                <th scope="row"><form:label path="imageWidthSize">이미지크기</form:label></th>
                <td>
                    <form:label path="imageWidthSize">가로</form:label>
                    <form:input path="imageWidthSize" size="2" maxlength="3" cssClass="specWidth70px"/>px
                    <form:label path="imageHeightSize" cssClass="marginLeft_10">세로</form:label>
                    <form:input path="imageHeightSize" size="2" maxlength="3" cssClass="specWidth70px"/>px
                </td>
            </tr>
            <tr>
                ...
                <td><input type="hidden" name="resizeUseAt" id="resizeUseAt" value="<c:out value="${bannerZone.resizeUseAt}"/>" />
                    ...
                    <div class="marginTop_5">
                        ...
                        <select id="resizeType" name="resizeType" class="marginLeft_10">
