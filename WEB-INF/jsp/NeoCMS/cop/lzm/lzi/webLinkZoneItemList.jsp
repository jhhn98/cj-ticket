...
<div class="card card--bgcolor p-search">
        <form:form modelAttribute="linkZoneItemVO" name="linkZoneItemVO" method="get">
    ...
    <div>
        <div class="margin_t_10 small"><span>총 배너 <em><c:out value="${paginationInfo.totalRecordCount}"/></em></span> &nbsp;&nbsp; <span>페이지 <em><c:out value="${paginationInfo.currentPageNo}"/> / <c:out value="${paginationInfo.totalPageCount}"/></em></span> </div>
    </div>
    <div id="link_box">
        <ul class="link_info">
<c:forEach var="result" items="${linkZoneItemList}" varStatus="status">
	<c:if test="${result.linkUrl ne '' and result.linkUrl ne null}">
            <li>
                <a href="<c:out value="${tsu:xssLinkFilter(result.linkUrl)}"/>" title="새창" target="_blank">
                    <span class="title"><c:out value="${result.linkZoneIemNm}"/></span>
    ...
    <div class="p-wrap bbs bbs__list">
        <div class="p-pagination">
            <div class="p-page">
        	<ui:pagination paginationInfo="${tsu:xssPagingFilter(paginationInfo)}" type="board" jsFunction="./selectLinkZoneItemWebList.do?key=${tsu:xssLinkFilter(key)}&amp;linkZoneNo=${tsu:xssLinkFilter(linkZoneItemVO.linkZoneNo)}&amp;pageUnit=${tsu:xssLinkFilter(linkZoneItemVO.pageUnit)}&amp;searchCnd=${tsu:xssLinkFilter(linkZoneItemVO.searchCnd)}&amp;searchKrwd=${tsu:xssLinkFilter(linkZoneItemVO.searchKrwd)}&amp;pageIndex=" />
            </div>
        </div>
    </div>
