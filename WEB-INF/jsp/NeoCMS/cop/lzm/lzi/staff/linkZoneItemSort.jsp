...
<div class="p-wrap">
	...
	<div class="col-20">
		...
		<form:form modelAttribute="linkZoneItemVO" name="linkZoneItemVOForm" action="./sortLinkZoneItem.do" onsubmit="IdxSubmit(this)">
			...
			<tr>
				<td colspan="2">
					...
					<c:forEach var="result" items="${linkZoneItemList}" varStatus="status">
						<option value="<c:out value="${result.linkZoneIemNo}"/>"><c:out value="${result.linkZoneIemNm}"/></option>
					</c:forEach>
					</select>
				</td>
			</tr>
		...
		<div class="row p-relative">
			...
			<div class="col-9 right">
				<a href="./selectLinkZoneItemList.do?key=<c:out value="${key}"/>&amp;siteId=<c:out value="${linkZoneItemVO.siteId}"/>&amp;linkZoneNo=<c:out value="${linkZoneItemVO.linkZoneNo}"/>&amp;pageUnit=<c:out value="${linkZoneItemVO.pageUnit}"/>&amp;searchCnd=<c:out value="${linkZoneItemVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${linkZoneItemVO.searchKrwd}"/>&amp;searchShowSe=<c:out value="${linkZoneItemVO.searchShowSe}"/>&amp;pageIndex=<c:out value="${linkZoneItemVO.pageIndex}"/>"  class="p-button p-button--small"><span>목록</span></a>
...
<script nonce="NEOCMSSCRIPT">
	function index_up() {
		var form = document.getElementById('linkZoneItemVO');
		var SelectIndex = form.idx.selectedIndex;
		if( SelectIndex < 0 ) {
			alert('링크 항목을 선택하여 주세요.');
			return false;
		}
		if( SelectIndex ) {
