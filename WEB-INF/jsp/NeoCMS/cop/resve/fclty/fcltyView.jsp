...
<head>
    ...
    <title><c:out value="${menuInfo.naviList[0].menuNm}"/> &gt; ${menuInfo.cntntsNm} 상세보기</title>
</head>
<body>
...
<div class="p-wrap">
    ...
    <table class="p-table">
        ...
        <tr>
            <th scope="row">시설명 / 운영상태</th>
            <td>
                <c:out value="${fcltyVO.fcltyNm}"/> / <c:out value="${operSttusMap[fcltyVO.operSttus]}"/>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <c:out value="${svcTyMap[fcltyVO.svcTyCd]}"/>
            </td>
        </tr>
        <tr>
            <th scope="row">지역</th>
            <td>
                <c:out value="${areaGuMap[fcltyVO.areaGuCd]}"/>
                <c:if test="${fcltyVO.areaGuCd == 'SDGU'}">
                    <c:out value="${sdguMap[fcltyVO.areaEmdCd]}"/>
                </c:if>
                <c:if test="${fcltyVO.areaGuCd == 'HDGU'}">
                    <c:out value="${hdguMap[fcltyVO.areaEmdCd]}"/>
                </c:if>
                <c:if test="${fcltyVO.areaGuCd == 'CWGU'}">
                    <c:out value="${cwguMap[fcltyVO.areaEmdCd]}"/>
                </c:if>
                <c:if test="${fcltyVO.areaGuCd == 'SWGU'}">
                    <c:out value="${swguMap[fcltyVO.areaEmdCd]}"/>
                </c:if>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                이용일 <em class="em_black"><c:out value="${fcltyVO.reqstClosCnt}"/></em>일 전까지 신청가능
                <span class="p-table__content padding_l_10">
                    <svg width="20" height="25" fill="#202e70" focusable="false"><use xlink:href="/common/images/program/p-icon.svg#info-circle"></use></svg>
        ...
        <tr>
            ...
            <td>
                이용일 <em class="em_black"><c:out value="${fcltyVO.canclClosCnt}"/></em>일 전까지 취소가능
                <span class="p-table__content padding_l_10">
                    <svg width="20" height="25" fill="#202e70" focusable="false"><use xlink:href="/common/images/program/p-icon.svg#info-circle"></use></svg>
        ...
        <tr>
            ...
            <td>
                <c:if test="${empty fcltyVO.fdrmCloseDayArr}">없음</c:if>
                <c:forEach var="result" items="${fcltyVO.fdrmCloseDayArr}" varStatus="status">
        ...
        <tr>
            ...
            <td>
                <c:out value="${fcltyVO.mtLmttTime}"/> 시간
                <span class="p-table__content padding_l_10">
                    <svg width="20" height="25" fill="#202e70" focusable="false"><use xlink:href="/common/images/program/p-icon.svg#info-circle"></use></svg>
                    <em class="em_black">'0'인 경우 제한없음</em>
                </span>
        ...
        <tr>
            ...
            <td>
                최소 : <c:out value="${fcltyVO.nmprMinCnt}"/> 명, 최대 : <c:out value="${fcltyVO.nmprMaxCnt}"/> 명
                <span class="p-table__content padding_l_10">
                    <svg width="20" height="25" fill="#202e70" focusable="false"><use xlink:href="/common/images/program/p-icon.svg#info-circle"></use></svg>
                    <em class="em_black">'0'인 경우 제한없음</em>
                </span>
        ...
        <tr>
            ...
            <td>
                <pre><c:out value="${fcltyVO.detailCn}"/></pre>
            </td>
        </tr>
        <tr>
            <th scope="row">유의사항</th>
            <td>
                <pre><c:out value="${fcltyVO.noticeCn}"/></pre>
            </td>
        </tr>
        <tr>
            <th scope="row">환불정책</th>
            <td>
                <pre><c:out value="${fcltyVO.refundPlcyCn}"/></pre>
            </td>
        </tr>
        <tr>
            <th scope="row">이미지</th>
        ...
        <tr>
            ...
            <td>
                <c:out value="${fcltyVO.aditIem1}"/><br/>
                <c:out value="${fcltyVO.aditIem2}"/>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <c:if test="${fcltyVO.resInqUseYn == 'N'}">
                    거주지조회 사용안함
                </c:if>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <c:if test="${fcltyVO.useYn == 'Y'}">
                    표시
                </c:if>
                <c:if test="${fcltyVO.useYn == 'N'}">
                    숨김
                </c:if>
            </td>
        </tr>
    ...
    <div class="h3">시설 일정 정보</div>
    <table class="p-table">
        <caption>시설 일정 상세보기</caption>
        <colgroup>
            <col class="w15p">
            <col />
        </colgroup>
        ...
        <tr>
            ...
            <td>
                <c:forEach var="result" items="${fcltyDeList}" varStatus="status">
                    <c:if test="${status.first}">
                        <c:out value="${tsu:toDateFormat(result.fcltyDe, 'yyyyMMdd', 'yyyy-MM-dd')}"/>
                    </c:if>
        ...
        <tr>
            ...
            <td><c:out value="${fcltySchdVO.unitHour}"/>시간<c:if test="${fcltySchdVO.unitMin > 0}"> <c:out value="${fcltySchdVO.unitMin}"/>분</c:if> 단위로 운영</td>
        </tr>
        <tr>
            <th scope="row">기본시설시간</th>
            <td>
                <c:out value="${tsu:toDateFormat(fcltySchdVO.baseBgnHm,'HHmm','HH:mm')}"/> ~ <c:out value="${tsu:toDateFormat(fcltySchdVO.baseEndHm,'HHmm','HH:mm')}"/>
                <c:choose>
                    <c:when test="${!empty fcltySchdVO.lunchBgnHh && !empty fcltySchdVO.lunchEndHh}">
                        (점심시간 : <c:out value="${fcltySchdVO.lunchBgnHh}:00"/> ~ <c:out value="${fcltySchdVO.lunchEndHh}:00"/>)
                    </c:when>
                    <c:otherwise>
                        (점심시간 없음)
                    </c:otherwise>
        ...
        <tr>
            ...
            <td>
                <table class="w50p">
                    ...
                    <c:if test="${fn:length(fcltyTmList) == 0}">
