...
<head>
    ...
    <title><c:out value="${menuInfo.naviList[0].menuNm}"/> &gt; ${menuInfo.cntntsNm} &gt; 일정관리 등록</title>
</head>
<body>
...
<div class="p-wrap">
    ...
    <form:form modelAttribute="fcltySchdVO" id="fcltySchdPdVO" name="fcltySchdPdVO" method="post" action="addFcltySchdPd.do" onsubmit="return fn_addFcltySchdPdCheck(this);" style="display:none;">
        <fieldset>
            ...
            <table class="p-table" id="fcltySchdPdTable">
                <caption>시설 일정 등록 - 시설기간(규칙 접수)</caption>
    ...
    <form:form modelAttribute="fcltySchdVO" id="fcltySchdDeVO" name="fcltySchdDeVO" method="post" action="addFcltySchdDe.do" onsubmit="return fn_addFcltySchdDeCheck(this);" style="display:none;">
        <fieldset>
            <legend>시설 일정 등록</legend>
            <table class="p-table" id=fcltySchdDeTable">
                <caption>시설 일정 등록 - 일자별선택(불규칙 접수)</caption>
                <c:forEach var="result" items="${fcltySearchVO.paramsMap}">
                    <input type="hidden" name="<c:out value="${result.key}"/>" value="<c:out value="${result.value}"/>"/>
                </c:forEach>
                <c:forEach var="result" items="${fcltySearchVO.paramsMapMng}">
...
<script>
    ...
    <%-- PD 입력된 항목 유효성 체크 --%>
    function fn_addFcltySchdPdCheck(form){
    ...
    <%-- DE 입력된 항목 유효성 체크 --%>
    function fn_addFcltySchdDeCheck(form){
