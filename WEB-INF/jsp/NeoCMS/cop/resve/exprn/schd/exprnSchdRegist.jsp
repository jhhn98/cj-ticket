...
<head>
    ...
    <title>${menuInfo.cntntsNm} > 체험 일정 등록</title>
</head>
<body>
...
<div class="p-wrap">
    ...
    <form:form modelAttribute="exprnSchdVO" id="exprnSchdPdVO" name="exprnSchdPdVO" method="post" action="addExprnSchdPd.do" onsubmit="return fn_addExprnSchdPdCheck(this);" style="display:none;">
        <fieldset>
            <legend>체험 일정 등록</legend>
            <table class="p-table" id="exprnSchdPdTable">
                <caption>체험 일정 등록 - 체험기간(규칙 접수)</caption>
            ...
            <div class="row margin_t_20">
                ...
                <div class="col-12 right">
                    <input type="submit" class="p-button edit" value="등록">
                </div>
            </div>
        </fieldset>
    </form:form>

    <form:form modelAttribute="exprnSchdVO" id="exprnSchdDeVO" name="exprnSchdDeVO" method="post" action="addExprnSchdDe.do" onsubmit="return fn_addExprnSchdDeCheck(this);" style="display:none;">
        <fieldset>
            <legend>체험 일정 등록</legend>
            <table class="p-table" id=exprnSchdDeTable">
                <caption>체험 일정 등록 - 일자별선택(불규칙 접수)</caption>
                <c:forEach var="result" items="${exprnSearchVO.paramsMap}">
                    <input type="hidden" name="<c:out value="${result.key}"/>" value="<c:out value="${result.value}"/>"/>
                </c:forEach>
                <c:forEach var="result" items="${exprnSearchVO.paramsMapMng}">
...
<script>
    ...
    function fn_renderTimeTable(exprnTmList) {
        ...
        for (var i=0; i<exprnTmList.length; i++) {
            ...
            html += tdTimeList(name, row, '4');
            html += tdTimeList(name, row, '5');
            html += tdTimeList(name, row, '6');
            html += tdTimeList(name, row, '7');
            html += tdTimeList(name, row, '1');

            html += '</tr>';
        }
        $('#exprnSchdPdVO #dayAllCheck').show();
        $('#exprnSchdPdVO .dayAll').prop('checked', true);
        $tbody.find("tr:not(:first)").remove();
        $tbody.append(html);
    }
    ...
    function tdTimeList(name, row, weekday) {
    ...
    // PD 유효성 체크
    function fn_addExprnSchdPdCheck(form){
    ...
    // DE 유효성 체크
    function fn_addExprnSchdDeCheck(form){
