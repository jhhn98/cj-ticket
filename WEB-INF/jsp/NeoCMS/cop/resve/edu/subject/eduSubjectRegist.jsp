...
<div class="p-wrap">
	...
	<form:form id="eduSubjectVO" action="./addEduSubject.do" method="post" modelAttribute="eduSubjectVO" onsubmit="return checkSubmit(this)" enctype="multipart/form-data">
	<fieldset>
		<legend>등록정보 작성</legend>
		<input type="hidden" name="key" value="${key}"/>
		<input type="hidden" name="prgSe" value="EDU"/>
		<c:if test="${not empty eduSubjectVO.paramsMapOther}">
			<c:forEach var="map" items="${eduSubjectVO.paramsMapOther}">
	...
	<table class="p-table">
		...
		<tr>
			...
			<td>
				<input type="text" id="subjectNm" name="subjectNm" value="" class="p-input w50p" maxlength="50" />
		...
		<tr>
			...
			<td>
				<span class="p-form-checkbox">
					<input type="checkbox" id="target" name="target" value="ALL" class="p-form-checkbox__input"/>
					<label for="target" class="p-form-checkbox__label">전체</label>
				</span> &nbsp; 
				<c:forEach var="cbox" items="${targetMap}" varStatus="sts">
				<span class="p-form-checkbox">
					<input type="checkbox" name="targetCdList" id="target${sts.index+1}" value="${cbox.key}" class="p-form-checkbox__input ctarget"/>
		...
		<tr class="tr_eduAmt" id="eduAmt_frm" style="display:none;">
			<th scope="row"><label for="eduAmt">수강료 금액</label> <span class="p-form__required--icon">필수</span></th>
			<td>
				<input type="text" id="eduAmt" name="eduAmt" value="${eduSubjectVO.eduAmtComa}" class="p-input w5p" maxlength="10" /> 원
				<%-- <span class="text_red margin_l_20">※ "0"을 입력하시면 무료로 표시 됩니다.</span> --%>
...
function fn_checkDplctSubjectNm() {
	var frm = document.getElementById('eduSubjectVO');
...
$(document).ready(function() {
    ...
    $("#target").click(function(){
        if($(this).is(":checked")){
            $('input[name="targetCdList"]').prop("checked",true);
        } else {
    ...
    $('.ctarget').click(function() {
        ...
        var ctlen = $('.ctarget:checked').length;
        if(etlen == ctlen) {
