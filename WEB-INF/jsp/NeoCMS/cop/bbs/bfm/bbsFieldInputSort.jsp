...
<div class="contents">
    ...
    <form:form modelAttribute="bbsFieldSortVO" name="bbsFieldSortVOForm" action="updateBbsFieldInputSort.do" onsubmit="IdxSubmit(this)">
        ...
        <div class="topMargin">
            <select name="idx" style="height:260px; width:100%; border:solid 1px #528ED8" size="16">
                <c:forEach var="result" items="${bbsFieldList}" varStatus="status">
                    <option value="<c:out value="${result.fieldNo}"/>"><c:if test="${result.viewMngOnlyShowAt ne '0'}">[관리자]</c:if><c:out value="${result.fieldUseNm}"/>(<c:out value="${result.fieldNm}"/>-<c:out value="${result.fieldCode}"/>)</option>
                </c:forEach>
            </select>
        </div>
