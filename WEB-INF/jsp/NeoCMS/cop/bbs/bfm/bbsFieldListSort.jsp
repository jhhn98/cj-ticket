...
<head>
	...
	<title>게시판 필드 목록 순서 변경</title>
	<meta name="decorator" content="neo" />
	<script nonce="NEOCMSSCRIPT">
		function index_up() {
			var form = document.getElementById('bbsFieldSortVO');
...
<div class="page_location">
	...
	<i class="sign_gt">메뉴 위치 구분 기호 ></i>
	<span class="depth4_title"><c:out value="${bbsSkin.skinNm}"/> 목록 순서 변경</span>
</div>
<div class="tab_navigation">
	<a href="./updateBbsInfoView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=" "><span>설정관리</span></a>
	<a href="./updateBbsFieldView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>필드관리</span></a>
	<a href="./updateBbsFieldListSortView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class="current"><span>목록 순서 변경</span></a>
	<a href="./updateBbsFieldInputSortView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>입력/보기 순서 변경</span></a>
	<a href="./selectBbsCategoryList.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>카테고리관리</span></a>
	<a href="./updateBbsDescriptionView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>설명관리</span></a>
	<a href="./selectBbsAuthorList.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>권한관리</span></a>
	<a href="./updateBbsAtchFieldView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" class=""><span>첨부파일필드 관리</span></a>
    <a href="./updateBbsBanView.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>&amp;pageUnit=<c:out value="${bbsFieldSortVO.pageUnit}"/>&amp;searchCnd=<c:out value="${bbsFieldSortVO.searchCnd}"/>&amp;searchKrwd=<c:out value="${bbsFieldSortVO.searchKrwd}"/>&amp;pageIndex=<c:out value="${bbsFieldSortVO.pageIndex}"/>" ><span>악성사용자 차단</span></a>
</div>
<div class="contents">
    <p class="info_text marginBottom_10">표시중인 항목만 순서변경이 가능하며, 표시되지 않는 항목은 맨 마지막으로 자동 이동 됩니다.</p>
    <form:form modelAttribute="bbsFieldSortVO" name="bbsFieldSortVOForm" action="updateBbsFieldListSort.do" onsubmit="IdxSubmit(this)">
        <form:hidden path="bbsNo"/>
        <form:hidden path="idxData"/>
        <input type="hidden" name="pageUnit" value="<c:out value="${bbsFieldSortVO.pageUnit}"/>"/>
        <input type="hidden" name="searchCnd" value="<c:out value="${bbsFieldSortVO.searchCnd}"/>"/>
        <input type="hidden" name="searchKrwd" value="<c:out value="${bbsFieldSortVO.searchKrwd}"/>"/>
        <input type="hidden" name="pageIndex" value="<c:out value="${bbsFieldSortVO.pageIndex}"/>"/>
        <select name="idx" style="height:260px; width:100%; border:solid 1px #528ED8" size="16">
            <c:forEach var="result" items="${bbsFieldList}" varStatus="status">
                <option value="<c:out value="${result.fieldNo}"/>"><c:if test="${result.listMngOnlyShowAt ne '0'}">[관리자]</c:if><c:out value="${result.fieldUseNm}"/>(<c:out value="${result.fieldNm}"/>-<c:out value="${result.fieldCode}"/>)</option>
            </c:forEach>
        </select>
