...
<div class="page_location">
	...
	<i class="sign_gt">메뉴 위치 구분 기호 ></i>
	<span class="depth4_title">그룹 사용자 등록</span>
</div>
...
<div class="contents">
	<div class="arrangementWrap data_info">
		...
		<div class="arrRight">
			<div class="table_search">
				<form:form modelAttribute="bbsAuthorGroup" name="bbsAuthorGroup" method="get" action="./addBbsAuthorGroupView.do">
					<form:hidden path="bbsNo"/>
					<form:hidden path="groupSe"/>
				...
				<table>
					...
					<c:forEach var="result" items="${userInfoList}" varStatus="status">
						<tr>
							...
							<td class="textACenter"><a href="./addBbsAuthorGroup.do?bbsNo=<c:out value="${bbsVO.bbsNo}"/>&userId=<c:out value="${result.userId}"/>&groupSe=<c:out value="${bbsAuthorGroup.groupSe}"/>" class="iconTextButton write small"><span>등록</span></a></td>
						</tr>
	...
	<div class="arrangementWrap data_button">
		<div class="arrLeft">
			<ui:pagination paginationInfo = "${tsu:xssPagingFilter(paginationInfo)}" type="text" jsFunction="./addBbsAuthorGroupView.do?siteId=${tsu:xssLinkFilter(bbsVO.siteId)}&amp;bbsNo=${tsu:xssLinkFilter(bbsVO.bbsNo)}&amp;groupSe=${tsu:xssLinkFilter(bbsAuthorGroup.groupSe)}&amp;pageUnit=${tsu:xssLinkFilter(bbsAuthorGroup.pageUnit)}&amp;searchCnd=${tsu:xssLinkFilter(bbsAuthorGroup.searchCnd)}&amp;searchKrwd=${tsu:xssLinkFilter(bbsAuthorGroup.searchKrwd)}&amp;pageIndex=" />
		</div>

		<div class="arrRight">
			<a href="./selectBbsAuthorList.do?siteId=<c:out value="${bbsVO.siteId}"/>&amp;bbsNo=<c:out value="${bbsVO.bbsNo}"/>" class="iconButton"><span>권한 목록</span></a>
		</div>
	</div>
</div>
