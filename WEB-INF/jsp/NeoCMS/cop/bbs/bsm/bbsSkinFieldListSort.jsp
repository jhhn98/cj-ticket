...
<div class="tab_navigation">
	...
	<a href="./updateBbsSkinFieldListSortView.do?skinId=${bbsSkin.skinId}" class="current"><span>목록 순서 변경</span></a>
...
<div class="contents">
    ...
    <form:form modelAttribute="bbsSkinFieldSortVO" name="bbsFieldSortVOForm" action="updateBbsSkinFieldListSort.do" onsubmit="IdxSubmit(this)">
        ...
        <select name="idx" style="height:260px; width:100%; border:solid 1px #528ED8" size="16">
            <c:forEach var="result" items="${bbsSkinFieldList}" varStatus="status">
                <option value="<c:out value="${result.skinFieldNo}"/>"><c:if test="${result.listMngOnlyShowAt ne '0'}">[관리자]</c:if><c:out value="${result.fieldUseNm}"/></option>
            </c:forEach>
        </select>
