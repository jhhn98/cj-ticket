...
<div class="arrangementWrap marginBottom_5 data_info">
    ...
    <div class="arrRight">
        <div class="table_search">
            <form:form modelAttribute="userInfoVO" name="userInfoVO" method="get" onsubmit="return fn_ajax_search_submit(this.id);return false;">
...
<table data-list="employeeList">
    ...
    <c:forEach var="result" items="${userInfoList}" varStatus="status">
        <tr>
            ...
            <td><button type="button" onclick="selectEmployee(this);return false; " data-emplcode="<c:out value="${result.emplCode}"/>" data-deptcode="<c:out value="${result.deptCode}"/>" data-emplnm="<c:out value="${result.emplNm}"/>" data-deptnm="<c:out value="${result.deptNm}"/>" data-userid="<c:out value="${result.userId}"/>" data-usernm="<c:out value="${result.userNm}"/>" class="iconButton check"><span>선택</span></button></td>
        </tr>
    </c:forEach>
...
<div class="textACenter marginTop_15 pagingWrap">
    <ui:pagination paginationInfo = "${tsu:xssPagingFilter(paginationInfo)}" type="neoTreePop" jsFunction="/neo/selectPopUserInfoList.do?pageUnit=${tsu:xssLinkFilter(employee.pageUnit)}&amp;searchCnd=${tsu:xssLinkFilter(employee.searchCnd)}&amp;searchKrwd=${tsu:xssLinkFilter(employee.searchKrwd)}&amp;pageIndex=" />
</div>
