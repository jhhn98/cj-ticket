                        ...
                        <#list bbsFieldListShowList as bbsFieldListShow>
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    ...
                                    <c:if test="@@{bbsNttVO.bbsManageMode eq true || result.deleteAt ne 'Y'}"><a href="<c:out value="@@{nttUrl}"/>" target="${bbsInfo.nttLinkTrget}" class="p-media__link"></c:if>
                                        ...
                                        <div class="p-media__body">
                                            <div class="p-media__heading p-media__heading--ellipsis">
                                                <em class="p-media__heading-text"><c:out value="@@{result.nttSj}"/></em>
                                                <c:if test="@@{result.newIcon}"><span class="p-icon p-icon__new">새글</span></c:if>
                                                <c:if test="@@{result.coolIcon}"><span class="p-icon p-icon__hot">핫이슈</span></c:if>
                                            </div>
                                            <div class="p-author__info">
                                        <span class="p-split">
                                            <em class="skip">작성일 :</em>
                                            <time><c:out value="@@{result.frstRegisterPnttmYMD}"/></time>
                                        </span>
                                                <span class="p-split">
                                            <em class="skip">조회 :</em>
                                            <i class="p-icon p-icon__eye"></i>
                                            <span data-mask="#,##0" data-mask-reverse="true"><fmt:formatNumber value="@@{tsu:xssNumberFilter(result.rdcnt)}" pattern="#,###"/></span>
                                        </span>
                                            </div>
                                            <div class="p-media__content">
                                                <c:choose>
                                                    <c:when test="@@{fn:length(result.nttCn) > 260}">
                                                        <c:out value="@@{fn:substring(result.nttCn,0,259)}"/>...
                                                    </c:when>
                                                    <c:otherwise><c:out value="@@{result.nttCn}"/></c:otherwise>
                                                </c:choose>
                                            </div>
                                        </div>
                                    </#if>
